...
import com.demons.media.Builder.AlbumBuilder;
...
import com.demons.media.models.sticker.entity.TextStickerData;
import com.demons.media.ui.PuzzleActivity;
...
import com.demons.media.utils.result.Result;
...
public class Gallery {
    ...
    //AlbumMedia的返回数据Key
    public static final String RESULT_PHOTOS = "keyOfAlbumMediaResult";
    public static final String RESULT_SELECTED_ORIGINAL = "keyOfAlbumMediaResultSelectedOriginal";

    /**
     * 预加载
     * 调不调用该方法都可以，不调用不影响AlbumMedia正常使用
...
//*************************bitmap功能***********************************/
     ...
     *                             为同一界面。false则在AlbumMedia内部完成，正常需求直接写false即可。
    ...
    public static void startPuzzleWithPhotos(FragmentActivity act, ArrayList<Photo> photos,
        ...
        act.setResult(Activity.RESULT_OK);
        Result.get(act).startPuzzleWithPhotos(photos, puzzleSaveDirPath, puzzleSaveNamePrefix
