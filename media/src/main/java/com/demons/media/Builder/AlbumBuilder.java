package com.demons.media.Builder;
...
import com.demons.media.ui.PhotosActivity;
...
public class AlbumBuilder {
    ...
    private static final String TAG = "com.huantansheng.AlbumMedia";
     ...
     * @return AlbumBuilder AlbumMedia的实例
     */
    ...
    public void start(int requestCode) {
        setSettingParams();
        launchAlbumMediaActivity(requestCode);
    }
    ...
    public void start(SelectCallback callback) {
        ...
        if (null != mActivity && null != mActivity.get() && mActivity.get() instanceof FragmentActivity) {
            Result.get((FragmentActivity) mActivity.get()).startEasyPhoto(callback);
            return;
        }
        if (null != mFragmentV && null != mFragmentV.get()) {
            Result.get(mFragmentV.get()).startEasyPhoto(callback);
            return;
        }
     ...
     * @param requestCode startActivityForResult的请求码
     */
    private void launchAlbumMediaActivity(int requestCode) {
        if (null != mActivity && null != mActivity.get()) {
            PhotosActivity.start(mActivity.get(), requestCode);
            return;
        }
        if (null != mFragment && null != mFragment.get()) {
            PhotosActivity.start(mFragment.get(), requestCode);
            return;
        }
        if (null != mFragmentV && null != mFragmentV.get()) {
            PhotosActivity.start(mFragmentV.get(), requestCode);
        }
    }
    ...
    private static void clear() {
        com.demons.media.result.Result.clear();
        Setting.clear();
        instance = null;
    }
